...
**System (changed 2026-08-11):** when Jim asks for something next, the first thing to do is give it the next available number and reorder the list around that — number = actual order, not a derived sort by completion date or start date. No more auto-resorting whenever a status changes.
...
**Re-completions:** if an item was marked 🟩 before but turned out not to actually be finished (a screwup, not new scope), it moves back to ❌ with a note: "(Previously shown as complete — [what was actually missing].)"
...
Files: lib/rateLimiter.ts, app/api/audit/route.ts
...
#0065 — ❌ GBP Agent: vendor account setup — Google Places API + billing, Veriphone, Lob (PC-GBP-1) — start: 2026-08-03. Jim's own action, cannot be done by Claude. Blocks #52, #53, #56, #57.

#0066 — ❌ GBP Agent: draft pingclose_gbp_audits migration, get Jim's separate explicit yes, run it (PC-GBP-2) — start: 2026-08-03. SQL already drafted and explained column-by-column in the 2026-08-03 session.

#0067 — ❌ GBP Agent: NAP extraction from existing HTML scrape (PC-GBP-3) — start: 2026-08-03. No dependencies, safe to build first. Must ship with hardened JSON.parse (try/catch, size cap, explicit field picks, no object spreading).

#0068 — ❌ GBP Agent: discovery agent — find candidate GBP listing, never auto-picks below confidence threshold (PC-GBP-4) — start: 2026-08-03. Depends on #49, #51.

#0069 — ❌ GBP Agent: profile data agent — Places Details, field-masked for cost control (PC-GBP-5) — start: 2026-08-03. Depends on #49, #52.

#0070 — ❌ GBP Agent: category alignment agent — the critical GBP-vs-website mismatch check, most important finding in the feature (PC-GBP-6) — start: 2026-08-03. Depends on #53.

#0071 — ❌ GBP Agent: website/GBP consistency agent — NAP + schema diff (PC-GBP-7) — start: 2026-08-03. Depends on #51, #53.

#0072 — ❌ GBP Agent: phone compliance agent — Veriphone line-type lookup (PC-GBP-8) — start: 2026-08-03. Depends on #49, #53. Correctly-hedged copy only — Google does not ban mobile/VoIP numbers, only premium-rate ones.

#0073 — ❌ GBP Agent: address compliance agent — Lob, PO-Box/CMRA hard-fail + residential informational-only (PC-GBP-9) — start: 2026-08-03. Depends on #49, #53.

#0074 — ❌ GBP Agent: competitor gap agent, reuses existing DataForSEO local-pack logic, no new API call (PC-GBP-10) — start: 2026-08-03. Depends on #53.

#0075 — ❌ GBP Agent: findings normalizer/scoring, critical-mismatch cap so it can't be buried (PC-GBP-11) — start: 2026-08-03. Depends on #54-58.

#0076 — ❌ GBP Agent: orchestrator + GBP-specific spend cap that applies even to VIP (PC-GBP-12) — start: 2026-08-03. Depends on #51-59.

#0077 — ❌ GBP Agent: API route + wire into /api/audit's existing after() block (PC-GBP-13) — start: 2026-08-03. Depends on #60.

#0078 — ❌ GBP Agent: report page section, category-mismatch warning pinned above the score (PC-GBP-14) — start: 2026-08-03. Depends on #50, #59.

#0079 — ❌ GBP Agent: security hardening rollup — re-check every item before shipping (PC-GBP-15) — start: 2026-08-03. Cuts across #51, #60.

#0080 — ❌ GBP Agent: Level 2 owner-authorized deep scan — future, do NOT start yet (PC-GBP-16) — start: 2026-08-03. Deferred until #49-63 are live and Jim explicitly opens this gate.
...
#0094 — ❌ PageSpeed retry fix coded but not tested — affects report reliability (PC-C12) — start: 2026-07-16

#0095 — ❌ pingclose.com's own /pricing page has generic WebPage schema instead of Offer/PriceSpecification (no tag yet) — start: 2026-08-11. Found while checking pingclose.com for FAQ/pricing content as a sanity check before the citywideheatingair.com audit. `app/pricing/page.tsx` displays a real $495 price but its JSON-LD is just `@type: WebPage` — same category of miss (schema not matching real page content) that's being flagged on client audits. Small, contained fix: swap in `Product`/`Offer` schema with the real price. Not urgent, but should get its own number so it doesn't get lost — same standard we're holding client sites to.
Files: app/pricing/page.tsx

#0096 — ❌ citywideheatingair.com — above-the-fold cleanup, now actually audited (client site, not a code task) — start: 2026-08-11 · audited: 2026-08-11. Jim's read going in was "too many pictures" — actually confirmed there are none: zero `<img>` tags and zero CSS background-images above the fold on mobile (matches the original report's site-wide count of only 2 images total, anywhere). The real bloat is structural, verified via live DOM/computed-style inspection at 375x812 mobile viewport, not guessed:
  1. **Visible nav menu eats 173px** (~19% of the mobile viewport) before any hero content — confirmed via computed styles: two other `<nav>` variants are correctly hidden (`display:none`, `visibility:hidden` for the off-canvas drawer), but a third renders inline, `display:block`/`visibility:visible`, 10 links.
  2. **A complete 5-field contact form (First/Last/Phone/Email/Message + Submit) is embedded directly in the hero**, immediately below the H1 and CTA button — not a lightweight prompt, the full form markup ships in the initial viewport.
  3. **That embedded hero form carries the identical broken Turnstile captcha already found on `/coupons/`** — confirmed via matching `data-sitekey="0x4AAAAAABm7inzwZLqOu8kC"` — meaning the dead-lead-form bug from #0093 is not isolated to the coupons page, it's on the homepage hero, the site's single most important lead-capture surface.
Separately, no-CDN (already tracked, drives #0087-#0089) is a real but *independent* contributing factor to the same 9.2s LCP — it affects asset delivery speed, not how much markup the browser has to build before paint. Both compound the same number through different mechanisms; neither explains the other.
Recommendation once WP access exists: replace the embedded hero form with a single lightweight CTA button (removes a redundant instance of the currently-broken form from the page that matters most, and cuts real above-the-fold weight), and fix the Turnstile bug once — it's the same instance reused sitewide, not two separate bugs. Full context in projects/pingclose/clients/citywideheatingair.com/NOTES.md.
