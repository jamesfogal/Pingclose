...
**⭐ JIM'S TOP PRIORITY (pinned 2026-08-11, not a numbering exception): #0099 — the audit tool doesn't detect missing HTML page-caching.** Surfaced from citywideheatingair.com, confirmed as a real, systematic gap affecting every audit, not a one-off. Jim's words: "a consistent screw-up from the company that built our sites... it now has a very high priority to me." Pinned here instead of renumbered, so it stays visible without reviving the priority-rank system Jim killed above.
...
#0087 — ❌ Client-Fix Agent: script-defer-agent — defer/async render-blocking CSS/JS via edge proxy (PC-FIX-3) — start: 2026-08-11. Surfaced from citywideheatingair.com audit: 17 render-blocking scripts/stylesheets, ~10.9s wasted, confirmed as the actual root cause of its 9.2s LCP (TTFB is 4ms — the entire delay sits between server response and first paint, not the server itself). No WordPress admin access needed — runs as a reverse-proxy edge layer in front of the client's origin, extending the existing Edge Agent pipeline (`lib/agents/superAgent/`) rather than a new build. **Blocked on #41/PC-EDGE3**: `edge-deploy-agent` has zero implementation and the CDN/reverse-proxy mechanism for getting in front of a client's own domain is undecided — that decision has to be made before this can ship to any real client site.

#0088 — ❌ Client-Fix Agent: unused-asset-agent — strip/defer unused CSS/JS via edge proxy (PC-FIX-4) — start: 2026-08-11. Addresses the "107KB unused JS / 16KB unused CSS" report finding, same site. Same access model and same #41/PC-EDGE3 blocker as PC-FIX-3.

#0089 — ❌ Client-Fix Agent: image-optimize-agent — in-process compression via edge proxy, zero new external API calls (PC-FIX-5) — start: 2026-08-11. Uses Sharp locally rather than a third-party compression API, keeping the file within the 2-external-API-call limit. Same #41/PC-EDGE3 blocker.

#0090 — ❌ Client-Fix Agent: schema-injector-agent — constrained to real page content only, never fabricates schema (PC-FIX-6) — start: 2026-08-11. Must never generate FAQPage/Review/Pricing schema for content that doesn't exist on the page — that's false structured data to Google and risks a penalty, and directly contradicts PingClose's own "reveal findings, don't fabricate" rule. Where the underlying content doesn't exist (e.g., citywideheatingair.com has none), this agent's job is to output a content-gap flag for a human, not auto-generate placeholder schema. Depends on #0086 (need the page-existence signal first to know whether there's real content to mark up).
