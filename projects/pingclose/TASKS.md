...
#0097 — 🟩 `Bash(git push*)` removed from `.claude/settings.json`'s deny list — start: 2026-08-11 · completed: 2026-08-11. This was a deliberate hard gate implementing CLAUDE.md's "never push without explicit approval" rule at the tool-permission layer, set up during an earlier superAgent session. Jim didn't recall it being added and asked for it removed; confirmed explicitly after being shown exactly what it does (a chat "yes" is now sufficient for pushes going forward, no separate technical block) before it was changed. `vercel deploy`/`netlify deploy` stay denied — only the git push rule was touched. Worth knowing: this lowers a real safety rail that was working as designed — nothing wrong happened because of it, it was removed on request.
